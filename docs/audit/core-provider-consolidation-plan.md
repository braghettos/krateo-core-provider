...
**4.3 `composition-version` label name (#102/#104). → DECIDED (hold the name); consolidation deferred with a clearer path.**
Name held: keep `krateo.io/composition-version` (upstream #102 rename reverted by #104). **Consolidating the constant** was thought to just need the plumbing baseline aligned, but attempting that (below) revealed the plumbing modernization is itself **blocked by upstream `provider-runtime`**. So the cleaner path for 4.3 is a **small dedicated `braghettos/krateo-labels` module** both core-provider and CDC import — it sidesteps the plumbing version split entirely (a trivial constants-only module vs. a multi-repo baseline move). The CROSS-REPO CONTRACT comment stays until one of these lands.

**Follow-up — OTel-aligned logging (consolidate + OTLP export). → FOUNDATION SHIPPED.**
Production logging was already OTel-*model* (SeverityNumber, trace correlation, service.name) but the handler was **duplicated** in core-provider (`loghandler`) and unstructured-runtime (`NewOTelJSONHandler`), and logs were stderr-JSON only (not OTLP). Now that `provider-runtime` is forked, the shared handler lives there: **`braghettos/provider-runtime v1.2.2`** adds `logging.NewOTelJSONHandler` (consolidated OTel-model JSON) + `logging.NewOTelHandler` (tees the JSON stream **and** an otelslog bridge → global LoggerProvider, so logs export via OTLP once the binary installs one). It depends only on the OTel log **API** + otelslog (no SDK), so it stays tidy-clean. **Rollout — ✅ DONE (both islands).**
- **Step 1 (provider-runtime island):** `braghettos/provider-runtime v1.2.2` ships the consolidated `NewOTelJSONHandler` + `NewOTelHandler` (JSON tee + otelslog bridge); **core-provider** (`0370d2e`) drops its `loghandler` duplicate, adopts it, and installs the OTLP log SDK/exporter in `main` (`telemetry.SetupOTLPLogs`, gated `--otel-logs-enabled`). Binary verified emitting OTel-model JSON.
- **Step 2 (unstructured-runtime island):** `braghettos/unstructured-runtime v1.3.4` mirrors `NewOTelHandler` + `telemetry.SetupOTLPLogs` (the islands don't share a home, so the ~60-line tee is mirrored — same fragmentation as 4.3); **CDC** (`c618e36`) adopts it (gated `--otel-logs-enabled`). Binary verified emitting OTel-model JSON.
