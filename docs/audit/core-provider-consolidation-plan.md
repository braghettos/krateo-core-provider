...
Production logging was already OTel-*model* (SeverityNumber, trace correlation, service.name) but the handler was **duplicated** in core-provider (`loghandler`) and unstructured-runtime (`NewOTelJSONHandler`), and logs were stderr-JSON only (not OTLP). Now that `provider-runtime` is forked, the shared handler lives there: **`braghettos/provider-runtime v1.2.2`** adds `logging.NewOTelJSONHandler` (consolidated OTel-model JSON) + `logging.NewOTelHandler` (tees the JSON stream **and** an otelslog bridge → global LoggerProvider, so logs export via OTLP once the binary installs one). It depends only on the OTel log **API** + otelslog (no SDK), so it stays tidy-clean. **Rollout remaining:** core-provider adopts `NewOTelHandler` (drops its `loghandler` duplicate) + installs the OTLP log SDK/exporter in main (next to tracing); unstructured-runtime/CDC mirror the same in their island (they don't import provider-runtime). The SDK-side setup re-encounters the OTel log beta version matrix (`sdk/log`/`otlploghttp` v0.9.0).

**Follow-up — plumbing-baseline modernization (core-provider 1.7.x → 1.10.x). → UNBLOCKED (provider-runtime forked).**
The provider-runtime blocker below is now resolved by `braghettos/provider-runtime v1.2.2` (drops the `slogs/pretty` test dep). core-provider can now move to plumbing **v1.10.4** cleanly by adding `replace ... provider-runtime => braghettos/provider-runtime v1.2.2` alongside the plumbing bump. Original blocker, for the record:
