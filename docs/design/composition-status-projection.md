...
- **Secrets & RBAC.** RESTActions reference credentials (Secrets) for the target API, and
  neither the RESTAction nor its Secrets are chart resources — so **chart-inspector cannot
  derive this RBAC**; core-provider generates it from the declaration instead. See **§4.5**.
...
### 4.5 RBAC for status sources — chart-inspector can't cover it

Today the CDC's RBAC is derived by the **`chart-inspector`** service: the CDC queries it
(`URL_CHART_INSPECTOR`) for the **resources the chart deploys**, and RBAC is granted for
those. That mechanism is, by construction, **scoped to the chart** — and the new status
sources reach beyond the chart, so chart-inspector cannot (and should not) produce their
RBAC:

| Source | In the chart? | Chart-inspector covers RBAC? | Gap |
|---|---|---|---|
| `self`/`spec`/`status`, `helm` | n/a (in-hand) | n/a | none |
| `resourcesRefs` (managed GVRs) | **yes** (chart created them) | get/list — **but install RBAC may omit `watch`** | ensure **watch** verb for freshness |
| `apiRef` → RESTAction + its Secrets | **no** (referenced, not rendered) | **no — invisible to chart-inspector** | **entirely missing** |

So **`apiRef` is the real gap**: the `RESTAction` CR and the credential `Secret`s it
references are not chart resources, so chart-inspector never enumerates them and the CDC SA
gets no permission to read them. (This is the user-reported "chart inspector can't debug
RESTAction" — it is the wrong tool for it, on purpose.)

**Resolution: status-source RBAC is declaration-driven, generated by core-provider — not
by chart-inspector.** core-provider already builds the CDC's `ServiceAccount` +
`Role`/`RoleBinding` at bootstrap (`internal/tools/deploy`), so it adds, **additively**,
rules derived from the `CompositionDefinition`'s own declarations:

- per `resourcesRefs` GVR → `get;list;watch` on that `apiGroup`/`resource` (namespace- or
  label-scoped to the composition's resources);
- per `apiRef` → `get` on `restactions.templates.krateo.io` (the named RESTAction) and
  `get` on the Secret(s) it references for credentials.

This cleanly divides responsibility: **chart-inspector = chart resources; core-provider =
status-source resources** (which it alone knows about, because they live on the
`CompositionDefinition`). No chart-inspector change is needed or wanted.

> **Alternative for `apiRef` (Phase 3 decision):** rather than the CDC executing RESTActions
> in-process (needing the RBAC + Secret access above), **delegate execution to snowplow**,
> which already runs RESTActions under its own RBAC, and have the CDC consume the result.
> That avoids granting the CDC broad Secret access but adds a cross-service dependency.
> Decide when Phase 3 is scoped (§8/§11).
...
- **Status-source RBAC (§4.5)**: confirm the split — chart-inspector for chart resources,
  core-provider for declaration-derived status-source rules (incl. the `watch` verb for
  `resourcesRefs`). For `apiRef`, decide CDC-executes-RESTAction (needs Secret RBAC) vs
  delegate-to-snowplow (cross-service, no CDC Secret access).
