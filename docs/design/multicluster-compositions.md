...
> **Implementation status (2026-06-13):** First increment landed — a
> `CompositionDefinition` can now choose where the CDC (+ generated CRD + RBAC) is
> deployed via `spec.deploy.mode` (`Local` default | `Remote`) with
> `spec.deploy.kubeconfigRef` pointing at a native Secret (ESO-rotatable). Clients are
> resolved per-reconcile in `Connect()` (`internal/tools/clusterkube`): the
> CompositionDefinition + secrets + status stay on the management client; CRD/RBAC/CDC
> install on the target client. Hand-rolled, kubeconfig-only; no SA-token renewal yet.
>
> **Conversion webhook (resolved):** the CRD conversion config is now target-aware
> (`conversionConfFor`). Local targets use the in-cluster webhook Service (unchanged).
> Remote targets use a **URL-based** webhook when `CORE_PROVIDER_WEBHOOK_URL` (the
> externally reachable `/convert` endpoint of the management cluster) is set; otherwise
> they fall back to `NoneConverter` with a loud warning — strictly safer than an
> unreachable Service webhook, which would break every request needing conversion.
> To enable full multi-version conversion for remote targets, expose core-provider's
> conversion endpoint and set `CORE_PROVIDER_WEBHOOK_URL` (its TLS cert must match
> `cabundle`).
