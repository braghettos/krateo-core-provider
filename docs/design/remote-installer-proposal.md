...
3. **Bootstrap RBAC is broad.** A whole-umbrella install needs the hub kubeconfig to have **near-cluster-admin on the spoke during bootstrap** (CRDs, RBAC incl. `bind`/`escalate`, operators) — far broader than the single-cdc `remote-target-rbac.yaml`. Mitigate with ESO short-lived/scoped creds; lean on the fact that *steady-state* installs use the spoke's own SA.
